...
`INITIATED` is the initial state of a Payment Dispute. Customer has filed a dispute. Klarna will review the case and may request Partner to respond within deadline.
...
| `representment` | [`Representment`](../../doc/models/representment.md) | Optional | The representment case details if the dispute has transitioned to representment state. |
...
payment_dispute_state_change_initiated = PaymentDisputeStateChangeInitiated(
